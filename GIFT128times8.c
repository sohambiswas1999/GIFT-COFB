...
#if defined(_MSC_VER)
#define CGAL_ALIGN_16 __declspec(align(32))
#elif defined(__GNUC__)
#define CGAL_ALIGN_16 __attribute__((aligned(32)))
#endif

typedef __m256i V256;

#define SLliepi16(a, o) _mm256_slli_epi16(a, o)
#define SRliepi16(a, o) _mm256_srli_epi16(a, o)
#define XOR128(a, b) _mm256_xor_si256(a, b)
#define AND128(a, b) _mm256_and_si256(a, b)
#define OR128(a, b) _mm256_or_si256(a, b)
#define STORE128(a, b) _mm256_store_si256((V256 *)&(a), b)
#define LOAD128(a) _mm256_load_si256((const V256 *)&(a))
#define CONST128(a) _mm256_load_si256((const V256 *)&(a))
#define ROL16in128(a, o) _mm256_or_si256(_mm256_slli_epi16(a, 16 - (o)), _mm256_srli_epi16(a, o))
...
#define Shuffle(a)                                  \
    a = _mm256_shuffle_epi8(a, slice_shuffle_mask); \
...
#define Subcell(Slice0, Slice1, Slice2, Slice3)      \
    ...
    Slice3 = XOR128(Slice3, _mm256_set1_epi32(0xffffffff));
...
#define Add_RK_RC(Slice0, Slice1, Slice2, Slice3, U, V, RC) \
    ...
    Slice3 = XOR128(Slice3, _mm256_set_epi8(0x80, 0x00, 0x00, RC, 0x80, 0x00, 0x00, RC, 0x80, 0x00, 0x00, RC, 0x80, 0x00, 0x00, RC, 0x80, 0x00, 0x00, RC, 0x80, 0x00, 0x00, RC, 0x80, 0x00, 0x00, RC, 0x80, 0x00, 0x00, RC));

#define Key_Update(Round_number)                              \
    temp_1 = ROL16in128(K[(3 + (3 * Round_number)) % 4], 2);  \
    temp_2 = ROL16in128(K[(3 + (3 * Round_number)) % 4], 12); \
    K[(3 + (3 * Round_number)) % 4] = OR128(AND128(temp_1, _mm256_set1_epi32(0xffff0000)), AND128(temp_2, _mm256_set1_epi32(0x0000ffff)));
...
#define Shuffle_S0(a) _mm256_shuffle_epi8(a, shuffle_mask_S0);
#define Shuffle_S1(a) _mm256_shuffle_epi8(a, shuffle_mask_S1);
#define Shuffle_S2(a) _mm256_shuffle_epi8(a, shuffle_mask_S2);
#define Shuffle_S3(a) _mm256_shuffle_epi8(a, shuffle_mask_S3);
...
#define State2Var \
    S0 = LOAD128(State[0]), S1 = LOAD128(State[8]), S2 = LOAD128(State[16]), S3 = LOAD128(State[24]);

#define LoadKey \
    K[0] = LOAD128(Key[0]), K[1] = LOAD128(Key[8]), K[2] = LOAD128(Key[16]), K[3] = LOAD128(Key[24]);
...
CGAL_ALIGN_16 uint32_t Key[16 * 2] = {0x474c5f29, 0x474c5f29, 0x474c5f29, 0x474c5f29, 0x474c5f29, 0x474c5f29, 0x474c5f29, 0x474c5f29,
...
CGAL_ALIGN_16 uint32_t State[16 * 2] = {0x3bce9899, 0x3bce9899, 0x3bce9899, 0x3bce9899, 0x3bce9899, 0x3bce9899, 0x3bce9899, 0x3bce9899,
...
uint16_t maketrmask1[8 * 2] = {0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842, 0x0842};       // for first super diagonal
uint16_t maketrmask2[8 * 2] = {0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084};       // for 2nd super diagonal
uint16_t maketrmask3[8 * 2] = {0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008};       // for 3rd super diagonal
uint16_t make_shuffle_mask[8 * 2] = {0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0, 0x00f0}; // mask for swapmove used in interlacing shiffle applied to each slice

/*Subsequent epi shuffle masks*/
uint8_t makesliceshufflemask[16 * 2] = {0, 2, 1, 3, 4, 6, 5, 7, 8, 10, 9, 11, 12, 14, 13, 15,
                                        0 + 16, 2 + 16, 1 + 16, 3 + 16, 4 + 16, 6 + 16, 5 + 16, 7 + 16, 8 + 16, 10 + 16, 9 + 16, 11 + 16, 12 + 16, 14 + 16, 13 + 16, 15 + 16};

uint8_t make_shuffle_mask_S0[16 * 2] = {0, 3, 2, 1, 4, 7, 6, 5, 8, 11, 10, 9, 12, 15, 14, 13,
                                        0 + 16, 3 + 16, 2 + 16, 1 + 16, 4 + 16, 7 + 16, 6 + 16, 5 + 16, 8 + 16, 11 + 16, 10 + 16, 9 + 16, 12 + 16, 15 + 16, 14 + 16, 13 + 16};
uint8_t make_shuffle_mask_S1[16 * 2] = {1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12, 15, 14,
                                        1 + 16, 0 + 16, 3 + 16, 2 + 16, 5 + 16, 4 + 16, 7 + 16, 6 + 16, 9 + 16, 8 + 16, 11 + 16, 10 + 16, 13 + 16, 12 + 16, 15 + 16, 14 + 16};
uint8_t make_shuffle_mask_S2[16 * 2] = {2, 1, 0, 3, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15,
                                        2 + 16, 1 + 16, 0 + 16, 3 + 16, 6 + 16, 5 + 16, 4 + 16, 7 + 16, 10 + 16, 9 + 16, 8 + 16, 11 + 16, 14 + 16, 13 + 16, 12 + 16, 15 + 16};
uint8_t make_shuffle_mask_S3[16 * 2] = {3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12,
                                        3 + 16, 2 + 16, 1 + 16, 0 + 16, 7 + 16, 6 + 16, 5 + 16, 4 + 16, 11 + 16, 10 + 16, 9 + 16, 8 + 16, 15 + 16, 14 + 16, 13 + 16, 12 + 16};

uint8_t make_shuffle_mask_all_slices[16 * 2] = {0, 3, 2, 1, 5, 4, 7, 6, 10, 9, 8, 11, 15, 14, 13, 12,
